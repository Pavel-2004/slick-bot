...
export default function handler(req, res) {
    ...
    getDoc(workflowRef)
      .then(workflowData => {
        if (workflowData.exists()) {
          ...
          if (found) {
            workflow.emit("updateActiveChat")
          }
          else {
  ...
  workflow.on("updateActiveChat", () => {
    const activeChatRef = doc(db, "activeChats", req.body.activeChatId)
    updateDoc(activeChatRef, {
      status: "rejected",
      chats: [
        {from: "admin", type: "text", content: "We apologize, all our agents are currently busy.", reject: true},
        {from: "admin", type: "text", content: "Please tell us what message you want to leave?", reject: true},
        {from: "admin", type: "text", content: "Feel free to tell us a a way to contact you or any other information.", reject: true}
      ]
    })
      .then(activeChat => {
