...
import {useChatNav, useTicket} from "../../hooks/ticket";

export default function Chat() {
  const router = useRouter()
  const { pid } = router.query
  const [workflows, changeWorkflow, displayOptions, handleToggleOptions] = useNav(pid)
  const [chats, currentUserId, sendChat, currentChatId, setCurrentChatId] = useChatNav(pid)
  ...
  return (
    <>
        ...
        <div className={"lg:hidden w-full bg-black bg-nav flex flex-row justify-between"}>
          ...
          <div className={"flex flex-row h-full my-auto"}>
            <button
              ...
              className={"p-1 option-color text-white h-full text-sm ml-2 mr-2"}>Chat</button>
            <button
              onClick={() => {
        ...
        <div className={"hidden w-full bg-black bg-nav lg:flex flex-row justify-between " + (displayOptions ? "" : "curved-bottom-corners")}>
          ...
          <div className={"flex flex-row h-full my-auto h-full"}>
            <button
              ...
              className={"p-1 option-color text-white h-full text-2xl ml-2 mr-2"}>Chat</button>
            <button
              onClick={() => {
                window.location.href = "/chat-ticket/" + pid
              }
      ...
      <div className={"flex lg:hidden flex flex-row justify-center pt-20 p-1"}>
        <p>Kindly, get our app on your phone or login from a desktop or tablet.</p>
      </div>
      <div className={"hidden min-h-screen w-full lg:flex flex-col primary-background"}>
        ...
        <div className={"flex flex-1 flex-row mt-10 mb-10 pl-10 pr-10 min-h-full"} style={{overflow: "auto"}}>
          <div className={"flex flex-1 p-2"}>
            <DisplayActiveChat chats={chats} currentUserId={currentUserId} currentChatId={currentChatId} setCurrentChatId={setCurrentChatId} />
          </div>
          <div className={"flex flex-1 p-2"}>
            <DisplayChatCurr chats={chats} currentChatId={currentChatId} setCurrentChatId={setCurrentChatId} sendChat={sendChat} />
          </div>
          <div className={"flex flex-1 p-2"}>
            <DisplayChatControl />
          </div>
